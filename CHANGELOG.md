...
### Fixed
- **Sync's closing line reads correctly for single items.** It said `1 new transcripts` and `1 exchanges indexed`; each count now uses the singular for one (`1 new transcript · 1 exchange indexed · 1 error`).
...
### Added
- **Visible progress for long-running index and sync commands.** Foreground commands now show a terminal spinner; sync also reports which Conversation source it is processing. Non-interactive output receives clear start and completion lines instead.
...
### Added
- **Multilingual search by default.** The default embedding model is now `multilingual-e5-small` instead of an English-only model, so conversations in other languages — Ukrainian, mixed-language, anything non-English — finally embed and retrieve properly instead of being garbled by an English-only encoder. Search dimensions are unchanged, so there's no slowdown. Your existing index upgrades itself automatically: each background sync re-embeds a batch of older entries until the whole index is on the new model (the `status` and `stats` commands show how many are left). English-only users who want maximum English-retrieval quality can switch back by setting `EPISODIC_MEMORY_EMBED_MODEL=bge-small-en`.
- **Opt-in reranking for sharper results.** Add `--rerank` on the command line (or `rerank` via the in-editor tool, or set `EPISODIC_MEMORY_RERANK=1`) to run a second, more precise pass: the search pulls a larger pool of candidates, then a cross-encoder model re-reads each one against your query and reorders by true relevance. It's off by default because it downloads a small model on first use and adds a little latency, but when you want the best possible top results it's a meaningful step up.
- **Pick your embedding model.** A new `EPISODIC_MEMORY_EMBED_MODEL` setting lets you choose the model that powers semantic search (currently `multilingual-e5-small` or `bge-small-en`) without editing code; an unrecognized name falls back to the default with a clear warning. Switching models automatically re-embeds your index on the next syncs.
...
- **`index` now tells you the size of the job up front** — "To index: N conversation(s) across M project(s)." — before it starts the slow work, instead of leaving you guessing how much is left.

### Changed
- **Combined keyword + semantic search now ranks results by genuine merit.** Previously every semantic match was listed first and exact-text matches were tacked on after — so a strong keyword hit (an error code, a commit SHA) could rank below loosely-related semantic results. Search now fuses the two rankings (Reciprocal Rank Fusion), so a result that both rankings like rises to the top and keyword matches interleave where they belong.
