...
import { buildInstallDoctorReport, discoverInstalls, readVersionField, } from './install-doctor.js';
...
async function runInstallDoctor(args) {
    ...
    const statuses = installs.map(install => {
        const missing = findMissingFiles(install.path);
