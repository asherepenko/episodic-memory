 ...
 */
export function scanIndexScope(sourceDirs, excludedDirSet, limitToProject) {
    let projects = 0;
    let conversations = 0;
    for (const sourceDir of sourceDirs) {
        ...
        for (const entry of entries) {
            if (!entryIsDirectory(sourceDir, entry))
                continue;
            if (excludedDirSet.has(entry.name))
                continue;
            if (limitToProject && entry.name !== limitToProject)
                continue;
...
export async function indexConversations(limitToProject, maxConversations, concurrency = 1, noSummaries = false) {
    ...
    for (const sourceDir of sourceDirs) {
        for (const projectEntry of fs.readdirSync(sourceDir, { withFileTypes: true })) {
            if (!entryIsDirectory(sourceDir, projectEntry))
                continue;
