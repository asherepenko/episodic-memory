...
export function worktreeParent(name, known) {
    const direct = name.match(WORKTREE_SEGMENT);
    if (direct)
        return direct[1];
    if (!name.includes('-worktrees-'))
        return undefined;
    const collapsed = name.replace('-worktrees-', '-');
    let best;
    for (const candidate of known) {
        if (candidate === name || !collapsed.startsWith(candidate + '-'))
            continue;
        if (!best || candidate.length > best.length)
            best = candidate;
    }
    // Only the org dir matched ("org-worktrees-…" → "org"): too vague to fold.
    if (best && name.startsWith(best + '-worktrees-'))
        return undefined;
    return best;
}
export function groupProjects(members) {
