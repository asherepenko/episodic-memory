...
let selfTokenCache;
function selfToken() {
    if (selfTokenCache === undefined)
        selfTokenCache = getProcessStartToken(process.pid);
    return selfTokenCache;
}
function writeLockRecord(fd) {
    const record = { pid: process.pid, token: selfToken() };
...
export function acquireFileLock(lockPath) {
    ...
    try {
        ...
        if (err.code === 'EEXIST')
            return null; // another contender won the steal
        throw err;
    }
}
