 ...
 */
export function discoverInstalls(hostRoots) {
    const found = [];
 ...
 */
function manifestSummary(installPath) {
    ...
    const codex = existsSync(join(installPath, '.codex-plugin', 'mcp.json')) ||
        existsSync(join(installPath, '.mcp.json'));
    if (claude && codex)
        return 'Claude + Codex manifests present';
    if (claude)
        return 'Claude manifest only (Codex MCP config missing)';
    if (codex)
        return 'Codex manifest only (Claude plugin manifest missing)';
    return 'no host manifest — neither host can launch this install';
}
export function readVersionField(installPath) {
    try {
 ...
 */
export function buildInstallDoctorReport(statuses, { repairAttempted = false } = {}) {
    const lines = ['episodic-memory install integrity', ''];
