...
function readSidecar(jsonlPath: string): SidecarRead {
  let raw: string;
  try {
  ...
  if (obj.version !== SCHEMA_VERSION) return { kind: 'invalid' };
  if (!isKnownKind(obj.kind)) return { kind: 'invalid' };
  ...
  switch (obj.kind) {
    ...
    case 'complete':
      if (typeof obj.lastUpdated !== 'string') return { kind: 'invalid' };
...
function readLegacySummary(jsonlPath: string): SyncState | null {
  ...
  let jsonlStat: fs.Stats | null = null;
  try {
...
function migrate(jsonlPath: string, retryAll: boolean): SyncState {
  ...
  if (fromGlobal) {
    ...
    if (retryAll) return { kind: 'pending' };
...
export function openConversationSyncStateStore(opts?: { archiveDir?: string }): ConversationSyncStateStore {
  ...
  const load = (jsonlPath: string): SyncState => {
    ...
    if (result.kind === 'valid') {
      ...
      if (retryAll && result.state.kind === 'poison') return { kind: 'pending' };
      return result.state;
    }
    ...
    if (result.kind === 'invalid') return { kind: 'pending' };
