 ...
 */
function betterSqlite3InstallRoot(): string | null {
  try {
...
function runRebuild(installRoot: string, extraArgs: string[]): void {
...
function waitForBinding(timeoutMs: number): void {
 ...
 */
export function healNativeBinding(): void {
  ...
  try {
    if (bindingUsable()) return; // a prior holder already fixed it
    console.error(
