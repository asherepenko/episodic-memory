 ...
 */
export function shouldQueueForSummaryState(state: SyncState): boolean {
...
function shouldRefreshArchive(sourcePath: string, archivePath: string): boolean {
  try {
...
export async function indexConversations(
  ...
  for (const projectEntry of fs.readdirSync(sourceDir, { withFileTypes: true })) {
    ...
    if (!noSummaries) {
      ...
      const needsSummary = toProcess.filter(c =>
        shouldQueueForSummaryState(store.load(c.archivePath))
      );
...
export async function indexUnprocessed(concurrency: number = 1, noSummaries: boolean = false): Promise<void> {
  ...
  if (!noSummaries) {
    const needsSummary = unprocessed.filter(c =>
      shouldQueueForSummaryState(store.load(c.archivePath))
    );
    if (needsSummary.length > 0) {
      console.log(`Generating ${needsSummary.length} summaries (concurrency: ${concurrency})...\n`);
