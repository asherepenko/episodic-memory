...
import { runCodexSummary, buildCodexSummaryPrompt } from './codex-summarizer.js';
 ...
 */
export function isSdkErrorResult(message: unknown): boolean {
    ...
    for await (const message of iterator) {
      if (message && typeof message === 'object' && 'type' in message && message.type === 'result') {
        ...
        if (isSdkErrorResult(message)) {
          throw new SummarizerSdkError((message as any).subtype, (message as any).session_id);
        }
...
async function callCodex(prompt: string, sessionId: string, model?: string): Promise<string> {
  return runCodexSummary(sessionId, prompt, model);
}
      ...
      while (true) {
        ...
        if (m && (m as any).type === 'result') {
          ...
          if (isSdkErrorResult(m)) {
            throw new SummarizerSdkError((m as any).subtype, (m as any).session_id);
          }
...
export async function summarizeConversation(
  ...
  // Codex-native summarization through ephemeral `thread/fork` when the
  // conversation comes from Codex; falls through to Claude SDK on failure.
  ...
  if (codexSessionId) {
    try {
      ...
      log.warn(`  Codex summarizer unavailable, falling back to Claude: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
