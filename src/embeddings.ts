import type { FeatureExtractionPipeline } from '@huggingface/transformers-darwin-arm64';
 ...
 */
export function resolveEmbeddingModel(requestedKey: string | undefined): EmbeddingModel {
  if (requestedKey) {
    const found = EMBEDDING_MODELS[requestedKey];
    if (found) return found;
    console.error(
      `episodic-memory: unknown EPISODIC_MEMORY_EMBED_MODEL "${requestedKey}"; ` +
...
export async function initEmbeddings(): Promise<void> {
  if (!embeddingPipeline) {
    ...
    const createFeatureExtractionPipeline = pipeline as unknown as (
      ...
      model: string,
      options: { dtype: ModelDtype; progress_callback: () => void },
