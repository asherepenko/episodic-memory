...
Dispatch the `search-conversations` subagent. It searches archived Claude Code and Codex transcripts, reads the top 2–5 results in full, and returns a 200–1000 word synthesis with actionable insights and source file pointers. This is 50–100x cheaper than loading raw conversations into your context window — the agent distills only what's relevant rather than flooding context with full transcripts.
...
If the harness exposes the `search-conversations` agent, dispatch it the same way. Otherwise drive the MCP tools directly: search first, read the top 2–5 results, synthesize in your response, and include source pointers so the user can inspect the originals.
...
The underlying MCP tools are available when you need finer control than the agent provides, or when the current harness does not support agent dispatch:
- `mcp__plugin_episodic-memory_episodic-memory__search`
- `mcp__plugin_episodic-memory_episodic-memory__read`

These return raw results that dump full conversation excerpts into your context window. Use them only when you need precise line ranges or structured JSON output that the agent wouldn't produce. Keep the working set small — search first, read only the top 2–5 hits. For general historical lookups, the agent's synthesis is almost always sufficient.

See `MCP-TOOLS.md` for the complete API reference. `references/mcp-api.md` is the older reference, kept for backward compatibility.
